...
#define EF_SETTINGS_FIXES(X)                                                                                                                                                                                                                                      \
    ...
    X(Bool, bDoubleReleaseTripleComPtrTeardown, true, "(AE-only) fixes a use-after-free crash from a vanilla engine resource wrapper releasing its owned COM pointers twice")                                                                                     \
    ...
    X(Bool, bGCArrayCleanupBug, true, "fixes the Papyrus VM's incremental garbage collector sometimes exiting its array/object cleanup loop after a single entry, forgoing the rest of its time budget and causing progressive script lag/hitches")               \
